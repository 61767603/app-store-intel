...
const props = defineProps<{ filtered: GemApp[] }>()
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-xs border-collapse">
        ...
        <tbody class="divide-y divide-slate-800/30">
          <tr v-for="row in sorted" :key="row.app_id + '-' + row.category_id" class="hover:bg-slate-800/40 transition-colors" :class="{ 'bg-danger/[0.03]': row.rating_avg != null && row.rating_avg < 3.5 }">
