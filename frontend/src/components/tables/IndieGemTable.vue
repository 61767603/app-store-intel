...
import type { IndieGemApp } from '../../types/api'

const props = defineProps<{ filtered: IndieGemApp[] }>()
      ...
      <table class="w-full text-xs border-collapse">
        <thead class="sticky top-0 z-10">
          <tr class="border-b border-slate-700/30 bg-slate-800/30">
            ...
            <th class="px-3 py-2.5 text-[11px] text-left text-slate-500 font-medium uppercase tracking-wider">类型</th>
        ...
        <tbody class="divide-y divide-slate-800/30">
          <tr v-for="row in sorted" :key="row.app_id + '-' + row.category_id" class="hover:bg-slate-800/40 transition-colors">
            ...
            <td class="px-3 py-3 text-slate-500 text-xs max-w-[120px] truncate">{{ row.rev_type }}</td>
